...
int pr_line_table_column(int mode);
int pr_order_table_column(int mode);
int pr_order_line_table_column(int mode);
int pr_supp_table_column(int mode);
int pr_cust_table_column(int mode);
int pr_psupp_table_column(int mode);
int pr_part_table_column(int mode);
...
tdef tdefs[] =
{
	{"part.csv", "part table", 200000,
		pr_part, sd_part, PSUPP, 0},
	{"partsupp.csv", "partsupplier table", 200000,
		pr_psupp, sd_psupp, NONE, 0},
	{"supplier.csv", "suppliers table", 10000,
		pr_supp, sd_supp, NONE, 0},
	{"customer.csv", "customers table", 150000,
		pr_cust, sd_cust, NONE, 0},
	{"orders.csv", "order table", 150000,
		pr_order, sd_order, LINE, 0},
	{"lineitem.csv", "lineitem table", 150000,
		pr_line, sd_line, NONE, 0},
	{"orders.csv", "orders/lineitem tables", 150000,
		pr_order_line, sd_order, LINE, 0},
	{"part.csv", "part/partsupplier tables", 200000,
		pr_part_psupp, sd_part, PSUPP, 0},
	{"nation.csv", "nation table", NATIONS_MAX,
		pr_nation, NO_LFUNC, NONE, 0},
	{"region.csv", "region table", NATIONS_MAX,
...
gen_tbl (int tnum, DSS_HUGE start, DSS_HUGE count, long upd_num)
{
    ...
    if(tnum > -1){
		...
		{
		case LINE:
		    pr_line_table_column(upd_num);
		    break;
		case ORDER:
            pr_order_table_column(upd_num);
		    break;
  		case ORDER_LINE: 
			pr_order_line_table_column(upd_num);
			break;
		case SUPP:
			pr_supp_table_column(upd_num);
			break;
		case CUST:
			pr_cust_table_column(upd_num);
			break;
		case PSUPP:
            pr_psupp_table_column(upd_num);
		    break;
		case PART:
		    pr_part_table_column(upd_num);
		    break;
  		case PART_PSUPP: 
			pr_part_psupp_table_column(upd_num);
			break;
		case NATION:
			pr_nation_table_column(upd_num);
			break;
		case REGION:
			pr_region_table_column(upd_num);
			break;
		}
    }
